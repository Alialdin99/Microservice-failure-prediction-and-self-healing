...
helm install chaos-mesh chaos-mesh/chaos-mesh --namespace=chaos-mesh --set chaosDaemon.runtime=containerd --set chaosDaemon.socketPath=/run/containerd/containerd.sock --set controllerManager.enableFilterNamespace=true --set controllerManager.targetNamespace=default --set dashboard.enabled=true --set dashboard.service.type=NodePort --set dashboard.service.nodePort=30002
